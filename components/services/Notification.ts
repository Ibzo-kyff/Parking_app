 ...
 * ⚠️ Les messages de chat sont automatiquement EXCLUS
 */
export const getNotifications = async (
  userId?: number,
  parkingId?: number
  ...
  try {
    ...
    if (parkingId) params.append("parkingId", String(parkingId));
    ...
    return raw
      .filter((n: any) => ALLOWED_TYPES.includes(n.type))
      .map((n: any) => ({
        id: n.id,
        title: n.title,
        message: n.message,
