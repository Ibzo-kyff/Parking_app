  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter: belonging to parking and for rent
        if (parkingId) {
          items = items.filter(v => v.parking && Number(v.parking.id) === parkingId);
        }
        items = items.filter(v => v.forRent || v.status === 'rented' || v.forRent === true);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Véhicules louées</Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.noResults}>
          ...
          <Text style={styles.noResultsText}>Aucun véhicule loué trouvé</Text>
        </View>
      ) : (
        <FlatList
          data={filtered}
